...
LinkPicker.prototype.load_url = function(url) {
    var self = this;
    this.emit("info", "Loading: " + url);
    if (!url) return this.error("empty URL");
...
LinkPicker.prototype.check_type = function(content) {
    if (!content) return this.error("no content");
    if (content.search(/<rss/) > -1) {
...
LinkPicker.prototype.parse_rss = function(xml) {
    ...
    if (match) this.emit("info", "Match: " + match);
    feedparser.parseString(xml, function(err, meta, articles) {
        ...
        self.emit("info", "URLs: " + list.length + "/" + articles.length + " found in RSS");
...
LinkPicker.prototype.parse_html = function(html) {
    ...
    var list = Object.keys(check);
    self.emit("info", "URLs: " + list.length + "/" + as.length + " found in HTML");
